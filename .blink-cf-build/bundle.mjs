...
function runSimulation(params) {
  const predict = (metric) => {
    ...
    return c.base + params.taxRate * c.taxRate + params.subsidyAmount * c.subsidyAmount + params.fuelPrice * c.fuelPrice + params.publicSpending * c.publicSpending + params.interestRate * c.interestRate + params.envRegulation * c.envRegulation;
  };
  const addNoise = (val, factor) => val + (Math.random() - 0.5) * factor;
  ...
  const env = Math.max(0, Math.min(100, addNoise(predict("envImpact"), 1)));
  const satisfaction = Math.max(0, Math.min(100, addNoise(predict("satisfaction"), 2)));
  return {
...
app.post("/predict", async (c) => {
  ...
  const projectionData = months.map((month, i) => {
    ...
    return {
      ...
      inflation: Math.round((3 + (result.inflation - 3) * easeIn) * 100) / 100,
      employment: Math.round((92 + (result.employmentRate - 92) * easeIn) * 100) / 100,
      env: Math.round((50 + (result.envImpact - 50) * easeIn) * 100) / 100,
      satisfaction: Math.round((60 + (result.satisfaction - 60) * easeIn) * 100) / 100
    };
  });
  return c.json({
    ...result,
